...
use soma_core::{
    ActuatorPositions, ActuatorState, Lifecycle, Plant, PlantHealth, PositionApplication, ReachyActuatorState,
    ACTUATOR_COUNT,
};
...
pub const OPEN_DUCK_SCENE_PATH: &str = concat!(env!("CARGO_MANIFEST_DIR"), "/assets/open-duck-mini-v2/xmls/scene_flat_terrain.xml");
...
impl OpenDuckSimPlant {
    pub fn load(control_period: Duration) -> Result<Self, SimError> {
        let model = Arc::new(MjModel::from_xml(Path::new(OPEN_DUCK_SCENE_PATH)).map_err(|e| SimError::Load(e.to_string()))?);
        if (model.nq() as usize, model.nv() as usize, model.nu() as usize) != (21, 20, OPEN_DUCK_ACTUATOR_COUNT) {
            return Err(SimError::WrongModelSize { qpos: model.nq() as usize, qvel: model.nv() as usize, actuators: model.nu() as usize });
        }
        for (expected, name) in OPEN_DUCK_ACTUATOR_NAMES.into_iter().enumerate() {
            let actuator = model.actuator(name).ok_or(SimError::MissingActuator(name))?;
            if actuator.id != expected { return Err(SimError::WrongActuatorOrder { name, expected, actual: actuator.id }); }
        }
        let physics_schedule = PhysicsSchedule::new(control_period, model.opt().timestep).map_err(SimError::PhysicsSchedule)?;
        Ok(Self { data: MjData::new(model), physics_schedule, timeline: 1, sequence: 0 })
    }

    pub fn policy_decimation(&self) -> usize { 10 }
    pub fn physics_schedule(&self) -> PhysicsSchedule { self.physics_schedule }
    pub fn advance_physics_step(&mut self) { self.data.step(); }
    pub fn advance_control_period(&mut self) { for _ in 0..self.physics_schedule.substeps_per_control_period() { self.advance_physics_step(); } }
    pub fn model_dimensions(&self) -> (usize, usize, usize) { (self.data.model().nq() as usize, self.data.model().nv() as usize, self.data.model().nu() as usize) }
    fn positions(&self) -> [f32; OPEN_DUCK_ACTUATOR_COUNT] { std::array::from_fn(|i| self.data.joint(OPEN_DUCK_ACTUATOR_NAMES[i]).expect("validated joint").view(&self.data).qpos[0] as f32) }
    pub fn reset(&mut self) { self.data.reset(); self.timeline = self.timeline.wrapping_add(1); self.sequence = 0; }
}
