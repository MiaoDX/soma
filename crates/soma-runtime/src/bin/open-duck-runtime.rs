use soma_runtime::{
    ...
    open_duck::{
        decode_target, OPEN_DUCK_RT_SOCKET, OPEN_DUCK_RUNTIME_SOCKET, OPEN_DUCK_STATE_BYTES,
        OPEN_DUCK_STATE_KEY, OPEN_DUCK_TARGET_KEY,
    },
};
...
async fn main() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    loop {
        tokio::select! {
            ...
            received = socket.recv(&mut buffer) => { let size = received?; if size == OPEN_DUCK_STATE_BYTES { session.put(OPEN_DUCK_STATE_KEY, buffer[..size].to_vec()).await?; } }
