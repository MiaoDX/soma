...
completed: source checkout, compiled model inspection, provenance audit, ONNX metadata audit, official headless rollout, policy timing comparison, minimal vendored bundle, frozen Stage 0 case, user-approved baseline selection, two passing baseline reference runs, const-generic core seam with Reachy aliases, fixed OpenDuckSimPlant, and isolated Duck transport contract
next_slice: Stage 2 synthetic 50 Hz target through 500 Hz RT path
next_proof: ControlCore<14> target admission, ten 2 ms physics steps per policy frame, bounded transport, and clean shutdown
