...
current_slice: Stage 4 supervised launcher asserts applied lineage and latches observable RT expiry/rejection evidence
blocker: none after user-approved baseline selection
blocker_fingerprint: none
last_proven: supervised clean mode emits 101 targets with applied lineage, no rejection, and about 20 ms maximum message age; stall mode emits one target, observes expiry, and ends in measured hold with no stale renewal
...
next_proof: add root pose/contact metrics to the combined state, complete Soma-path gait rollouts against frozen floors, inject live duplicate/wrong-timeline/stale-source faults, and prove observer independence
